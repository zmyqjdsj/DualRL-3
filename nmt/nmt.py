import tensorflow as tf                #神经机器翻译（Neural machine translation, NMT）是最近提出的机器翻译方法。
import numpy as np                     #应该是pretrain一系列的东西
import time                            #f和g在这里面定义
import sys
import re
sys.path.append("..")
        ...
        while True:
            try:
                ...
                ops = [model.loss,
                       model.logits,
                       model.train_op, ]
                res = sess.run(ops, feed_dict=feed_dict)   #完成前项传播反向传播更新，每一百  sess等于一个函数的返回值

                n_batch += 1
                global_step += 1
                if n_batch % 100 == 0:
                    eval_loss = eval(model)  #评估损失函数
...
def inference(model, A, B, sess, args, src_test_iterator, src_test_next, src_vocab_rev, result_dir=None, step=None): #要改
    sess.run([src_test_iterator.initializer]) #需要改动
...
def get_nmt_direction(direction="0-1"): #fg转换
    if direction == "0-1":
        A = 0
        B = 1
    else:
        ...
        B = 0
    return A, B
def get_nmt_direction1(direction="0-2"): #新加的
    if direction == "0-2":
        A = 0
        C = 2
    else:
        ...
        C = 2
    return A, C
def get_nmt_direction2(direction="1-2"): #新加的
    if direction == "1-2":
        C = 2
        B = 1
    else:
...
if __name__ == "__main__":
    ...
    A, C = get_nmt_direction(args.nmt_direction)#新加的
    print("A=%s, C=%s" % (A, C))

    B, C = get_nmt_direction(args.nmt_direction)#新加的
    print("B=%s, C=%s" % (B, C))
    ...
    with tf.device("/cpu:0"):  # Input pipeline should always be placed on the CPU. 在cpu上输入？ 指定Session在第1块CPU上运行
        print("Use x'->y to update model f(x->y)") #需要改
    ...
    tf_config.gpu_options.per_process_gpu_memory_fraction = 0.4  #在现有的基础上吗？
    sess = tf.Session(config=tf_config)

    # === Train
    if args.mode == "train":#两个对称的model
        # Prepare for model saver
        print("Prepare for model saver") #模型的保存器
        args.nmt_model_save_dir = "%s/%s-%s/" % (args.nmt_model_save_dir, A, B) #要改，应该是实际名称加一个0-1
        ...
        dump_args_to_yaml(args, args.nmt_model_save_dir) #把地址转换成为静态的yaml的中间态

        # Initial and build model 一个训练模型一个推理模型？
    ...
    # === Inference
        ...
        final_model_save_path = "%s%s-%s/" % (args.nmt_model_save_dir, A, B) #要改
    ...
    # === Final inference (after dualRL training) 
        ...
        print("Prepare for model saver")
        final_model_save_path = "%s%s-%s/" % (args.final_model_save_dir, A, B) #要改
